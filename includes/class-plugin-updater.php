...
class MRC_Plugin_Updater {
    ...
    public function __construct($plugin_file) {
        ...
        add_filter('plugin_auto_update_setting_html', array($this, 'render_auto_update_toggle'), 10, 2);
    }
    ...
    /**
     * Ensure auto-update toggle appears for this plugin, even when WordPress does not provide default markup.
     *
     * @param string $html Existing markup generated by WordPress.
     * @param string $plugin_file Current plugin basename.
     * @return string
     */
    public function render_auto_update_toggle($html, $plugin_file) {
        if ($plugin_file !== $this->plugin_slug) {
            return $html;
        }

        // Reuse default output if WordPress already produced something meaningful.
        if ('' !== trim((string) $html)) {
            return $html;
        }
        ...
        $is_enabled = in_array($this->plugin_slug, $auto_update_plugins, true);

        $action = $is_enabled ? 'disable-plugin-auto-update' : 'enable-plugin-auto-update';
        $link_text = $is_enabled ? __('Disable auto-updates', 'mr-cloak') : __('Enable auto-updates', 'mr-cloak');
        ...
        $classes = 'toggle-auto-update ' . ($is_enabled ? 'enabled' : 'disabled');
